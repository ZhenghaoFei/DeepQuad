...
def main_quad(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_type, vf_params, animate=False):
    ...
    hover_position = np.asarray([50, 0, -50])
    ...
    sy_h1 = tf.nn.relu(dense(sy_ob_no, 1024, "h1", weight_init=normc_initializer(0.01))) # hidden layer
    sy_h1 = tf.layers.batch_normalization(sy_h1)

    sy_h2 =  tf.nn.relu(dense(sy_h1, 1024, "h2", weight_init=normc_initializer(0.01))) # hidden layer
    sy_h2 = tf.layers.batch_normalization(sy_h2)

    # sy_h3 =  tf.nn.relu(dense(sy_h2, 512, "h3", weight_init=normc_initializer(0.01))) # hidden layer
    # sy_h3 = tf.layers.batch_normalization(sy_h3)

    # sy_h4 =  tf.nn.relu(dense(sy_h3, 512, "h4", weight_init=normc_initializer(0.01))) # hidden layer
    # sy_h4 = tf.layers.batch_normalization(sy_h4)
    ...
    std_a = tf.sigmoid(dense(sy_h2, ac_dim, "logstdev",weight_init=normc_initializer(0.01)))*ac_lim # Mean control output

    # std_a =  tf.get_variable("logstdev", [ac_dim], initializer=tf.ones_initializer())
    # print mean_na
    # print std_a

    sy_sampled_ac0 = tf.expand_dims(sample_gaussian(mean_na[:, 0], std_a[:, 0]), axis=1)# sampled actions, used for defining the policy (NOT computing the policy gradient)
    sy_sampled_ac1 = tf.expand_dims(sample_gaussian(mean_na[:, 1], std_a[:, 1]), axis=1)# sampled actions, used for defining the policy (NOT computing the policy gradient)
    sy_sampled_ac2 = tf.expand_dims(sample_gaussian(mean_na[:, 2], std_a[:, 2]), axis=1)# sampled actions, used for defining the policy (NOT computing the policy gradient)
    
    sy_sampled_ac = tf.concat((sy_sampled_ac0, sy_sampled_ac1, sy_sampled_ac2), axis=1)
    # print sy_sampled_ac
    # sy_prob_n = (1.0/tf.sqrt((tf.square(std_a)*2*3.1415926))) * tf.exp(-0.5*tf.square((sy_ac_n - mean_na)/std_a))
    sy_prob_n0 = (1.0/(std_a[:, 0]*2.5067)) * tf.exp(-0.5*tf.square((sy_ac_n[:, 0] - mean_na[:, 0])/std_a[:, 0]))
    sy_prob_n1 = (1.0/(std_a[:, 1]*2.5067)) * tf.exp(-0.5*tf.square((sy_ac_n[:, 1] - mean_na[:, 1])/std_a[:, 1]))
    sy_prob_n2 = (1.0/(std_a[:, 2]*2.5067)) * tf.exp(-0.5*tf.square((sy_ac_n[:, 2] - mean_na[:, 2])/std_a[:, 2]))

    sy_logprob_n0 = tf.log(sy_prob_n0)
    sy_logprob_n1 = tf.log(sy_prob_n1)
    sy_logprob_n2 = tf.log(sy_prob_n2)
    sy_logprob_n = sy_logprob_n0 + sy_logprob_n1 + sy_logprob_n2
    ...
    old_std_a = tf.placeholder(shape=[None, ac_dim], name='old_std_a', dtype=tf.float32) # std_a BEFORE update (just used for KL diagnostic)
    # KL 
    ...
    # explore_loss = tf.reduce_mean(-0.5 * tf.log(2*3.141593*tf.square(std_a))+1)
    # sy_surr = - tf.reduce_mean(sy_adv_n * sy_logprob_n) + 1e-4*explore_loss# Loss function that we'll differentiate to get the policy gradient ("surr" is for "surrogate loss")
    ...
    for i in range(n_iter):
        ...
        while True:
            ...
            while True:
                ...
                # print ac
                ac = np.reshape(ac, (ac_dim,))
                # Quad
                act = [ac[0], ac[1], ac[2], 0, 0, 0]
...
if __name__ == "__main__":
    ...
    general_params = dict(gamma=0.99, animate=False, min_timesteps_per_batch=2500, n_iter=3000, initial_stepsize=1e-3)
