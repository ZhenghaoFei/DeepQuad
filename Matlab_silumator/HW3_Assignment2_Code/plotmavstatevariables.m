...
% Modified:  4/5/2016 - Gang
    ...
    % inverted pendulum
    pen_x       = uu(13);            % pendulum x displacement
    pen_y       = uu(14);            % pendulum y displacement               
    pen_vx      = uu(15);            % pendulum x velocity
    pen_vy      = uu(16);            % pendulum y velocity

    Va          = uu(13+4);            % airspeed (m/s)
    alpha       = 180/pi*uu(14+4);     % angle of attack (degrees)
    beta        = 180/pi*uu(15+4);     % side slip angle (degrees)
    wn          = uu(16+4);            % wind in the North direction
    we          = uu(17+4);            % wind in the East direction
    wd          = uu(18+4);            % wind in the Down direction
    pn_c        = uu(19+4);            % commanded North position (meters)
    pe_c        = uu(20+4);            % commanded East position (meters)
    h_c         = uu(21+4);            % commanded altitude (meters)
    Va_c        = uu(22+4);            % commanded airspeed (meters/s)
    alpha_c     = 180/pi*uu(23+4);     % commanded angle of attack (degrees)
    beta_c      = 180/pi*uu(24+4);     % commanded side slip angle (degrees)
    phi_c       = 180/pi*uu(25+4);     % commanded roll angle (degrees)   
    theta_c     = 180/pi*uu(26+4);     % commanded pitch angle (degrees)
    chi_c       = 180/pi*uu(27+4);     % commanded course (degrees)
    p_c         = 180/pi*uu(28+4);     % commanded body angular rate along x-axis (degrees/s)
    q_c         = 180/pi*uu(29+4);     % commanded body angular rate along y-axis (degrees/s)
    r_c         = 180/pi*uu(30+4);     % commanded body angular rate along z-axis (degrees/s)
    pn_hat      = uu(31+4);            % estimated North position (meters)
    pe_hat      = uu(32+4);            % estimated East position (meters)
    h_hat       = uu(33+4);            % estimated altitude (meters)
    Va_hat      = uu(34+4);            % estimated airspeed (meters/s)
    alpha_hat   = 180/pi*uu(35+4);     % estimated angle of attack (degrees)
    beta_hat    = 180/pi*uu(36+4);     % estimated side slip angle (degrees)
    phi_hat     = 180/pi*uu(37+4);     % estimated roll angle (degrees)   
    theta_hat   = 180/pi*uu(38+4);     % estimated pitch angle (degrees)
    chi_hat     = 180/pi*uu(39+4);     % estimated course (degrees)
    p_hat       = 180/pi*uu(40+4);     % estimated body angular rate along x-axis (degrees/s)
    q_hat       = 180/pi*uu(41+4);     % estimated body angular rate along y-axis (degrees/s)
    r_hat       = 180/pi*uu(42+4);     % estimated body angular rate along z-axis (degrees/s)
%    Vg_hat      = uu(43+4);            % estimated groundspeed
%    wn_hat      = uu(44+4);            % estimated North wind
%    we_hat      = uu(45+4);            % estimated East wind
%    psi_hat     = 180/pi*uu(46+4);     % estimated heading
%    bx_hat      = uu(47+4);            % estimated x-gyro bias
%    by_hat      = uu(48+4);            % estimated y-gyro bias
%    bz_hat      = uu(49+4);            % estimated z-gyro bias
    delta_e     = 180/pi*uu(50+4);     % elevator angle (degrees)
    delta_a     = 180/pi*uu(51+4);     % aileron angle (degrees)
    delta_r     = 180/pi*uu(52+4);     % rudder angle (degrees)
    delta_t     = uu(53+4);            % throttle setting (unitless)
    t           = uu(54+4);            % simulation time
    ...
    persistent pen_x_handle
    persistent pen_y_handle
    persistent pen_vx_handle
    persistent pen_vy_handle
  ...
  % first time function is called, initialize plot and persistent vars
    if t==0,
        ...
        subplot(2,2,1)
        hold on
        pen_x_handle = graph_y(t, pen_x, [], 'b');
        ylabel('pen_x')

        subplot(2,2,2)
        hold on
        pen_y_handle = graph_y(t, pen_y, [], 'b');
        ylabel('pen_y')

        subplot(2,2,3)
        hold on
        pen_vx_handle = graph_y(t, pen_vx, [], 'b');
        ylabel('pen_xv')

        subplot(2,2,4)
        hold on
        pen_vy_handle = graph_y(t, pen_vy, [], 'b');
        ylabel('pen_xy')
    % at every other time step, redraw state variables
       ...
       graph_y(t, pen_x, pen_x_handle);
       graph_y(t, pen_y, pen_y_handle);
